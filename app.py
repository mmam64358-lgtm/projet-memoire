...
def create_alert():
    ...
    cursor.execute(
        """
        INSERT INTO alerts (title, severity, description, lat, lng, status, zone_id, created_at)
        VALUES (%s, %s, %s, %s, %s, 'open', %s, %s)
        RETURNING id
        """,
        (title, severity, description, lat, lng, zone["id"] if zone else None, now_iso()),
    )
...
def optimize_ip():
    ...
    output += f"\n[System] Parameters loaded. Budget: €{budget}, Max Time: {horizon}m, Domino Limit: {domino}m"
    ...
    if total_cost == 0 and num_zones > 0:
        ...
        output += f"\nObjective Value: €{total_cost:.2f} utilized out of €{budget:.2f}"
        output += f"\n==========================================="
        output += f"\nDEPLOYMENT PLAN:"
        for p in plan:
            output += f"\n -> {p}"
...
def optimize_gp():
    ...
    output += f"\n[System] Parameters loaded. Goals: Damage <= {target_damage}, Cost <= {target_cost}."
    output += f"\n[System] Weights: W1={w1}, W2={w2}. Constraints: Budget=€{budget}, Horizon={horizon}m"
    ...
    for z in range(1, num_zones + 1):
        ...
        plan.append(f"Sector {z} ({severity}): Allocating €{allocation_cost}, Expc. Damage {allocation_damage}")
    ...
    if total_cost > budget:
        output += f"\n\n[WARNING] Global budget constraint €{budget} violated by €{total_cost - budget}!"
    ...
    output += f"\n[Result] Goal 2 (Cost): Expected €{total_cost} vs Target €{target_cost} (Dev: +{dev_cost})"
